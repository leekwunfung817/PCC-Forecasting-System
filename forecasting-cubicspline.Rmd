...
image_path <- "./forecasting-cubicspline/"
...
sql <- paste0("
  SELECT MIN(`Year`) `Min_Year`, YEAR(CURRENT_TIMESTAMP) this_year FROM `htaticketing`;
  ")
...
ticket_func <- function(year_) {
  ...
  title_ = paste(year_,"Interpolation")
  ...
  title_ = paste(year_,"First Derivative")
  image_path_ <- paste(image_path,title_,".png")
  png(image_path_)
  curve(
    fp,
  ...
  title_ = paste(year_,"Second Derivative")
  image_path_ <- paste(image_path,title_,".png")
  png(image_path_)
  curve(
    fpp,
...
for (i in starting_year:(this_year-1)) {
  ...
  A <- differentiate_list[[4]]
  print(paste("differentiate_list A :"))
  print(A)
  ...
  #print(paste("x_list:",x))
  #print(paste("y_list:",y))
...
model <- lm(y_last_36 ~ x_last_36)
slope <- coef(model)[2]



title_ = paste("Linear Regression")
image_path_ <- paste(image_path,title_,".png")
png(image_path_)
...
RichardsonExtrapolation_Annually_SpeedBased <- function (begin_x, begin_relative_x, begin_y, begin_speed, include_linear_regression) {
  ...
  if (include_linear_regression) {
    title_ <- paste(title_, " linear regression")
  }
  image_path_ <- paste(image_path,title_,".png")
  png(image_path_)
  # Plotting the graph
...
RichardsonExtrapolation_Annually_SpeedBased(last_data$x, begin_relative_x_, last_data$y, all_differentiate_value$fp, FALSE)
Sys.sleep(1)
prediction_equation = RichardsonExtrapolation_Annually_SpeedBased(last_data$x, begin_relative_x_, last_data$y, all_differentiate_value$fp, TRUE)
...
title_ = paste("Whole Picture Interpolation")
image_path_ <- paste(image_path,title_,".png")
png(image_path_)
...
sql <- paste0("
    with a as (
        SELECT
            DATE(MIN(`Performance Dates[Date]`)) min_date,
            DATE(MAX(`Performance Dates[Date]`)) max_date
        FROM
            `smoothing2`
    )
    select
          ...
          (
              DATEDIFF(
                  CONCAT(YEAR(`min_date`),'-01-01')
                  ,`min_date`
              )
              /
              DATEDIFF(
                  CONCAT(YEAR(`min_date`),'-01-01')
                  ,CONCAT(YEAR(`min_date`),'-12-31')
              )
          ) `min_date`,
          YEAR(`max_date`)+
          (
              DATEDIFF(
                  CONCAT(YEAR(`max_date`),'-01-01')
                  ,`max_date`
              )
              /
              DATEDIFF(
                  CONCAT(YEAR(`max_date`),'-01-01')
                  ,CONCAT(YEAR(`max_date`),'-12-31')
              )
          ) `max_date`
    from a;
  ")
date_range <- dbGetQuery(con, sql)
print("Date Range")
print(date_range)
print(date_range$min_date)
print(date_range$max_date)

max_date <- as.integer(date_range$max_date)
min_date <- as.integer(date_range$min_date)
...
sql <- paste0("
SELECT
      YEAR(`Performance Dates[Date]`)+
      (
          DATEDIFF(
              CONCAT(YEAR(`Performance Dates[Date]`),'-01-01')
              ,`Performance Dates[Date]`
          )
          /
          DATEDIFF(
              CONCAT(YEAR(`Performance Dates[Date]`),'-01-01')
              ,CONCAT(YEAR(`Performance Dates[Date]`),'-12-31')
          )
      ) `x`,
    `[Net_PAX]` `y`
FROM
...
fpp  <- function(year_) {
  ((fp(year_ + h) - fp(year_))/h)
}



curve(f,ylim=c(0, 3000))
...
title_ = paste("PAX Daily Whole Trending")
image_path_ <- paste(image_path,title_,".png")
png(image_path_)
